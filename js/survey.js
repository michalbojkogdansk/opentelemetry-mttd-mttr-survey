...
// Simple, reliable translation using data-t attributes
...
function translatePage() {
    ...
    // Handle input placeholders (stored as data-placeholder-t)
    document.querySelectorAll('input[placeholder], textarea[placeholder]').forEach(el => {
        if (!el.dataset.placeholderT) {
            el.dataset.placeholderT = el.getAttribute('placeholder');
        }
        el.setAttribute('placeholder', getText(el.dataset.placeholderT));
    });

    // Update navigation buttons (no data-t, managed by JS)
    const prevBtn = document.getElementById('prevBtn');
    const nextBtn = document.getElementById('nextBtn');
    const submitBtn = document.getElementById('submitBtn');
    const validateTokenBtn = document.getElementById('validateTokenBtn');

    if (prevBtn) prevBtn.textContent = getText('Wstecz');
    if (nextBtn) nextBtn.textContent = getText('Dalej');
